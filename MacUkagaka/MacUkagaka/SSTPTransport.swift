...
public enum SSTPError: Error {
    case protocolViolation
    case badRequest
}

/// Utility to construct a compliant SSTP request.
public func makeRequest(method: String, headers: [String: String], body: Data? = nil) -> String {
    var headers = headers
...
public class CFMessagePortTransport: NSObject, SSTPTransport {
    ...
    public func send(request: String, receiveTimeout: TimeInterval = 2.0) async throws -> String {
        semaphore.wait()
...
class CFMessagePortListener {
  ...
  init(portName: String, handler: @escaping (String) -> String) {
    ...
    let callout: CFMessagePortCallBack = { _, _, data, info in
        ...
        let response = listener.handler(message)
        return Unmanaged.passRetained(response.data(using: .utf8)! as CFData)
    }
